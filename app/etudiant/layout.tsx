...
export default function EtudiantLayout({ children }: { children: React.ReactNode }) {
    ...
    const dashboardHref = role === "ROLE_ADMIN"
        ? "/admin/dashboard"
    ...
    const adminLinks = [
        { name: "Tableau de bord", href: "/admin/dashboard", icon: LayoutDashboard, exact: true },
    ...
    const defaultLinks = [
        { name: "Tableau de bord", href: dashboardHref, icon: LayoutDashboard, exact: true },
        ...
        { name: "Chat", href: "/etudiant/chat", icon: MessageSquare, exact: false },
    ];
    ...
    const allLinks = [
        ...navLinks,
        { name: "Mon Profil", href: "/etudiant/profile", icon: User, exact: true },
    ...
    return (
        <div className="min-h-screen bg-[#f0f1f3] dark:bg-[#050505] font-sans text-[#333333] dark:text-slate-400 flex relative">
            ...
            <aside className={`
                ...
                w-64 bg-[#042954] dark:bg-slate-800 dark:border-r dark:border-slate-700/50 text-white flex flex-col shadow-xl z-50
            `}>
                <div className="p-6 border-b border-white/10 dark:border-slate-700/50 flex items-center justify-between">
                ...
                <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                    ...
                    {navLinks.map((link) => {
                        ...
                        return (
                            <Link
                                key={link.href}
                                href={link.href}
                                className={`flex items-center gap-3 px-4 py-3 rounded-lg font-medium transition-colors ${isActive ? 'bg-[#ffa000] text-white shadow-sm' : 'hover:bg-black/20 dark:hover:bg-black/40 text-gray-300 hover:text-white'}`}
                            >
                                <Icon size={20} />
                                {link.name}
                            </Link>
                    ...
                    {[
                        ...(role !== "ROLE_ADMIN" ? [{ name: "Mon Profil", href: "/etudiant/profile", icon: User }] : [])
                    ].map((link) => {
                        const Icon = link.icon;
                    ...
                    <div className="flex items-center gap-4 relative z-20">
                        ...
                        <NotificationBell />
                         {role === "ROLE_ADMIN" ? (
                            ...
                            <Link href="/etudiant/profile" className="flex items-center gap-3 border-l pl-4 hover:opacity-80 transition-opacity">
                                <div className="hidden sm:block text-right">
                                    <p className="text-sm font-bold text-[#333333] dark:text-slate-100">{userName}</p>
                                    <p className="text-xs text-gray-500 dark:text-slate-400 font-medium">{displayRole}</p>
                                </div>
