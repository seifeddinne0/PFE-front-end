...
import { LogOut, FileText, Settings, Bell, LayoutDashboard, BookOpen, UserX, Menu, X, Files, User } from "lucide-react";
...
export default function EnseignantLayout({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            try {
                ...
                if (data) {
                    setUserName(`${data.prenom || ""} ${data.nom || ""}`.trim() || "Enseignant");
    ...
    const navLinks = [
        ...
        { name: "Notes & Résultats", href: "/enseignant/notes", icon: FileText },
        { name: "Absences", href: "/enseignant/absences", icon: UserX },
        { name: "Documents", href: "/enseignant/documents", icon: Files },
    ];
    ...
    const pageTitle = activeLink ? activeLink.name : "Espace Enseignant";
                                ...
                                {userPhoto ? (
                                    <img 
                                        ...
                                        onError={(e) => {
                                            ...
                                            (e.target as HTMLImageElement).parentElement!.innerText = userName.charAt(0) || "E";
                                        }}
                                    />
                                ) : (
                                    userName.charAt(0) || "E"
