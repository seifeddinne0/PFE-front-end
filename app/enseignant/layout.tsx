...
export default function EnseignantLayout({ children }: { children: React.ReactNode }) {
    ...
    const allLinks = [
        ...navLinks,
        { name: "Mon Profil", href: "/enseignant/profile", icon: User, exact: true },
        ...
        <div className="min-h-screen bg-[#f0f1f3] dark:bg-[#050505] font-sans text-[#333333] dark:text-slate-400 flex relative">
            ...
            `}>
                ...
                <nav className="flex-1 px-4 py-6 space-y-2 overflow-y-auto">
                    ...
                    {[
                        ...(role !== "ROLE_ADMIN" ? [{ name: "Mon Profil", href: "/enseignant/profile", icon: User }] : [])
                    ].map((link) => {
                        const Icon = link.icon;
                    ...
                    <div className="flex items-center gap-4">
                        <ThemeToggle />
                         {role === "ROLE_ADMIN" ? (
                            ...
                            <Link href="/enseignant/profile" className="flex items-center gap-3 border-l pl-4 hover:opacity-80 transition-opacity">
                                <div className="hidden sm:block text-right">
                                    <p className="text-sm font-bold text-[#333333] dark:text-slate-100">{userName}</p>
                                    <p className="text-xs text-gray-500 dark:text-slate-400 font-medium">{displayRole}</p>
                                </div>
